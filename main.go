...
func main() {
	//NOTE: Name of PDF is invoice.pdf

	lineItems := []LineItem{
		{
			UnitName:       "Numo Chicken Liver Cat Food",
			PricePerUnit:   375, //in cents
			UnitsPurchased: 400,
		}, {
			UnitName:       "Feather Wand",
			PricePerUnit:   822, //in cents
			UnitsPurchased: 50,
		}, {
			UnitName:       "Custom Collar",
			PricePerUnit:   1455, //in cents
			UnitsPurchased: 50,
		}, {
			UnitName:       "SoftCat Comfy Cat Carrier with Removable Mat, Detachable Food Bowl, Side and Front Ventilation",
			PricePerUnit:   6085, //in cents
			UnitsPurchased: 50,
		}, {
			UnitName:       "FancyFeast Chicken Treats",
			PricePerUnit:   550, //in cents
			UnitsPurchased: 50,
		}, {
			UnitName:       "CleanFeet Kitty Litter",
			PricePerUnit:   2500, //in cents
			UnitsPurchased: 100,
		},
	}
	...
	//Banner - Logo
	pdf.ImageOptions("../Bisou.png", 248.0, 0+(bannerHt-(bannerHt/1.5))/2.0, 0, bannerHt/1.5, false, gofpdf.ImageOptions{
		ReadDpi: true}, 0, "")
	...
	pdf.MultiCell(124.0, lineHt*1.5, "(123) 456-7890\ncsiadat@scu.edu\ncsiadat.com", gofpdf.BorderNone, gofpdf.AlignRight, false)
	...
	pdf.MultiCell(124.0, lineHt*1.5, "123 Code World\nGo, Lang\n12345", gofpdf.BorderNone, gofpdf.AlignRight, false)

	//Summary - Billed To, Invoice #, Date of Issue
	_, sy := summaryBlock(pdf, xIndent, bannerHt+lineHt*2.0, "Billed To", "SJC Cares Shelter", "479 Lincoln Ave", "San Jose, CA, USA", "95043")
	summaryBlock(pdf, xIndent*2.0+lineHt*12.5, bannerHt+lineHt*2.0, "Invoice Number", "0000000001")
	summaryBlock(pdf, xIndent*2.0+lineHt*12.5, bannerHt+lineHt*6.25, "Date of Issue", "02/25/2024")
	...
	y = y + lineHt*.5
	trailerLine(pdf, x, y, "Total", total)
...
func lineItem(pdf *gofpdf.Fpdf, x, y float64, lineItem LineItem) (float64, float64) {
	...
	y = y + lineHt*1.75
	pdf.SetDrawColor(180, 180, 180)
	pdf.Line(xIndent-10.0, y, w-xIndent+10, y)
	return origX, y
}
